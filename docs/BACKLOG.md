...
- Question-aware emphasis (decided 2026-09-20, do in session 5 after hosting is up): the preset question picks which path the page points at — try_first/doctor → A with the lead-time box; how_long → B; appeal → B plus the appeal steps; cost → the cost column, C and D; free text → no emphasis. One green pointer line above the paths card ("You asked about a denial. Path B is the fastest route from where you are…"), the pointed-at row tinted, its details section open by default. Rows never reordered or hidden. Second part, design conversation with Ben first: a fifth row "E — Appeal the denial" shown only for the appeal question (written reason → peer-to-peer/resubmit → internal appeal with the Ohio/federal clocks → external review), built from the reference documents already in the corpus. Ben asked to be reminded of this in session 5.
